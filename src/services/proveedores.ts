...
import { importeFirmadoRecibido } from "./cfeTypes.js";
...
export function rankingProveedores(
  ...
  for (const r of recibidos) {
    ...
    // Las NC recibidas reducen la compra, el IVA crédito y las retenciones.
    // Ver `importeFirmadoRecibido`: el signo lo da el tipo, no el número.
    const firmado = (valor: number | null): number => importeFirmadoRecibido(r.tipo, valor);
