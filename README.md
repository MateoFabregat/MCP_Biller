...
- **Lectura (read-only):** consultas de comprobantes y datos DGI. Solo `GET`.
- **Escritura "con barreras":** emitir/anular comprobantes, crear clientes/
  productos y emitir/cancelar recibos (`POST`) — **deshabilitada por defecto** y
  protegida por dry-run + confirmación + gate de ambiente + idempotencia + audit.
...
| `BILLER_WRITE_ENABLED` | ❌ | `false` | Habilita ejecutar POST. Sin esto, solo dry-run. |
...
Para **habilitar escritura en test**: poné `"BILLER_WRITE_ENABLED": "true"`. Aun así,
cada emisión/anulación requiere el flujo dry-run → confirm con token.
...
Probá `biller_health_check` (mirá `mode`/`environment`), después un
`biller_emitir_comprobante` en **dry-run** y verificá el `confirmation_token`.
