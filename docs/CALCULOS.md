...
| Solo cuentan los CFE **fiscalmente válidos** (`solo_aceptados`, default `true`) | Incluye `Aceptado DGI` y `Envío no corresponde`: este último va por reporte diario y no es un rechazo. Pendientes, rechazados y desconocidos quedan fuera. |
...
`"Envío no corresponde"` suma desde septiembre de 2026 en todos los cálculos:
es una venta real informada por reporte diario. La decisión está centralizada en
`estaAceptado` y cubierta transversalmente por `tests/estadoDgi.test.ts`.
...
|---|---|---|
| Rechazos DGI | agrupa rechazados, pendientes y cualquier texto desconocido. `Envío no corresponde` no alerta porque es un CFE válido por reporte diario. El estado **ausente** —`null` o vacío— tampoco alerta: la falta del dato no es evidencia de rechazo | 🟢 exacta |
